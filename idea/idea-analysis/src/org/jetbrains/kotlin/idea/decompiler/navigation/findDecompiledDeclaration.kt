...
fun findDecompiledDeclaration(
    ...
    return decompiledFiles.asSequence().mapNotNull { file ->
        ...
        val library = when (moduleInfo) {
            is LibraryInfo -> moduleInfo.library
            is LibrarySourceInfo -> moduleInfo.library
            else -> null
        }
        library?.let { file.getResolutionFacade().findModuleDescriptor(LibraryInfo(project, it)) }?.let { libraryModule ->
            if (referencedModule.name != KotlinBuiltIns.BUILTINS_MODULE_NAME
